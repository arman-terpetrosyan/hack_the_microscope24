...
sims = {}
print(chip_df)
sim = squareform(1 - pdist(np.stack(chip_df['image data'].values), metric="cosine"))
print(sim)
...
sims['cluster'] = sim
print(pred)
print(np.unique(np.asarray(pred.values)))
df = pd.DataFrame({"chip ID": pred.keys(), 'cluster': pred.values()})
# df = df.sort_values(by=['chip ID']).replace({"chip ID": inv_d})
print(df)
print(results.join(df, how='outer'))
